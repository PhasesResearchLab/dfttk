...
from dfttk.debye.functions import (
    calculate_gruneisen_parameter,
    calculate_debye_temperatures,
    calculate_debye_integral_n3,
    calculate_entropies,
    calculate_helmholtz_energies,
...
gruneisen_x = 2 / 3
gruneisen_parameter = 2.1397031639791155

debye_temperatures = calculate_debye_temperatures(volumes, V0, B, atomic_mass, gruneisen_parameter, scaling_factor)
...
def test_calculate_gruneisen_parameter():
    ...
    expected_gru_param = 2.1397031639791155

    assert np.isclose(gruneisen_parameter, expected_gru_param, rtol=1e-5), f"Expected {expected_gru_param}, but got {gruneisen_parameter}"


def test_debye_temperatures():
    debye_temperatures = calculate_debye_temperatures(volumes, V0, B, atomic_mass, gruneisen_parameter, scaling_factor)
    ...
    assert np.allclose(debye_temperatures, expected_debye_temperatures, rtol=1e-5), f"Expected {expected_debye_temperatures}, but got {debye_temperatures}"


def test_calculate_debye_integral_n3():
    debye_integral = calculate_debye_integral_n3(x_array)
    ...
    assert np.allclose(debye_integral, expected_debye_integral, rtol=1e-5), f"Expected {expected_debye_integral}, but got {debye_integral}"


def test_calculate_entropies():
    entropies = calculate_entropies(temperatures, debye_temperatures[0], number_of_atoms)
    expected_entropies = np.array([0.00000000e00, 9.84493663e-05, 4.15416511e-04, 7.22940954e-04, 9.76604225e-04, 1.18610897e-03, 1.36283321e-03, 1.51502047e-03, 1.64837946e-03, 1.76692036e-03, 1.87353338e-03])
    assert np.allclose(entropies, expected_entropies, rtol=1e-5), f"Expected {expected_entropies}, but got {entropies}"


def test_calculate_helmholtz_energies():
    helmholtz_energies = calculate_helmholtz_energies(temperatures, debye_temperatures[0], number_of_atoms)
    expected_helmholtz_energies = np.array([0.24267095, 0.24006104, 0.21495901, 0.15759857, 0.07219976, -0.03625184, -0.16393316, -0.30800338, -0.4663115, -0.63718655, -0.81929879])
    assert np.allclose(helmholtz_energies, expected_helmholtz_energies, rtol=1e-5), f"Expected {expected_helmholtz_energies}, but got {helmholtz_energies}"


def test_calculate_heat_capacities():
    heat_capacities = calculate_heat_capacities(temperatures, debye_temperatures[0], number_of_atoms)
    expected_heat_capacities = np.array([0.0, 0.00025215, 0.00066319, 0.00083989, 0.00091781, 0.00095741, 0.00097995, 0.00099391, 0.00100312, 0.00100951, 0.00101411])
    assert np.allclose(heat_capacities, expected_heat_capacities, rtol=1e-5), f"Expected {expected_heat_capacities}, but got {heat_capacities}"


def test_process_debye_gruneisen():
    helmholtz_energies, entropies, heat_capacities = process_debye_gruneisen(
        number_of_atoms,
        volumes,
        temperatures,
        atomic_mass,
