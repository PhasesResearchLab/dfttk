...
class MongoDBStorage:
    ...
    def store_ev_curve(
        ...
        eos_df = eos_df.drop(
            columns=["config", "number_of_atoms", "volumes", "energies", "pressures"]
        )
        eos_df = eos_df[eos_df["EOS"] == eos]
    ...
    def store_fvib(self, object_id: str, harmonic_properties_fit: pd.DataFrame):
        ...
        polynomial_columns = [
            "f_vib_polynomial",
            "entropy_polynomial",
            "heat_capacity_polynomial",
        ]
        polynomial_data = harmonic_properties_fit_reset[polynomial_columns]
        f_vib_coefficients = polynomial_data["f_vib_polynomial"].to_list()
        s_vib_coefficients = polynomial_data["entropy_polynomial"].to_list()
        cv_vib_coefficients = polynomial_data["heat_capacity_polynomial"].to_list()
    ...
    def store_qha(self, object_id: str, quasi_harmonic_properties: pd.DataFrame):
        ...
        F0_list = quasi_harmonic_properties["F0"].values.tolist()
        ...
        qha_data = {
            "$set": {
                "properties.QHA": {
                    ...
                    "CTE": CTE_list,
                    "Cp": Cp_list,
                    "F EOS fit": {"EOS": "BM4", "constants": eos_dict},
                }
            }
        }
