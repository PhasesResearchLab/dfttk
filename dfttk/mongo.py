...
class MongoDBStorage:
    ...
    def __init__(
        self, path: str, connection_string: str, db="DFTTK", collection="community", config_ids = None
    ):
        ...
        if config_ids is None:
            config_folders = [
                ...
                and folder.startswith("config_")
            ]
            config_folders = natsorted(config_folders)
            config_object_ids = [ObjectId() for _ in range(len(config_folders))]
            self.config_ids = dict(zip(config_folders, config_object_ids))
        else:
    ...
    def store_ev_curve(
        ...
        vol_folders = [
            ...
            if os.path.isdir(os.path.join(config_path, f)) and f.startswith("vol")
        ]
        vol_folders = natsorted(vol_folders)

        # material
        poscar = Poscar.from_file(
            os.path.join(config_path, vol_folders[0], "POSCAR.1relax")
        )
        ...
        incar_relax = Incar.from_file(
            os.path.join(config_path, vol_folders_no_errors[0], "INCAR.1relax")
        )
        incar_static = Incar.from_file(
            os.path.join(config_path, vol_folders_no_errors[0], "INCAR.3static")
        )
        kpoints = Kpoints.from_file(
            os.path.join(config_path, vol_folders_no_errors[0], "KPOINTS.1relax")
        )
        ...
        potcar_path = os.path.join(config_path, vol_folders_no_errors[0], "POTCAR")
        ...
        for vol_folder in vol_folders:
            poscar = Poscar.from_file(
                os.path.join(config_path, vol_folder, "POSCAR.3static")
            )
        ...
        if object_id:
            result = self.collection.update_one(
                {"_id": ObjectId(object_id)}, {"$set": document}, upsert=True
            )
    ...
    def store_multiple_ev_curves(
        self, df: pd.DataFrame, eos_parameters_df: pd.DataFrame, eos: str
    ):
        config_folders = list(self.config_ids.keys())

        for config_folder in config_folders:
            object_id = self.config_ids[config_folder]
            config_name = config_folder.split("_")[1]
            config_df = df[df["config"] == config_name]
            multiplicity = str(config_df["multiplicity"].values[0])
            config_eos_parameters_df = eos_parameters_df[eos_parameters_df["config"] == config_name]
            config_path = os.path.join(self.path, config_folder)
            
            self.store_ev_curve(
                config_path=config_path,
                config_name=config_name,
                df=config_df,
                eos_parameters_df=config_eos_parameters_df,
                eos=eos,
                multiplicity=multiplicity,
                object_id=object_id,
            )
    ...
    def store_fvib_debye(self, object_id: str, debye_properties: pd.DataFrame):
        ...
        scaling_factor = debye_properties["scaling_factor"].unique()[0]
        gruneisen_x = debye_properties["gruneisen_x"].unique()[0]
        ...
        print(f"Document with ID {object_id} updated with fvib_debye_data.")
        
        return fvib_debye_data
    
    def store_multiple_fvib_debye(
        self, debye_properties: pd.DataFrame
    ):
        config_folders = list(self.config_ids.keys())
        ...
        for config_folder in config_folders:
            object_id = self.config_ids[config_folder]
            config_name = config_folder.split("_")[1]
            config_debye_properties = debye_properties[debye_properties["config"] == config_name]
            self.store_fvib_debye(
                object_id=object_id,
                debye_properties=config_debye_properties,
            )
