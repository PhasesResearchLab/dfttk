...
def eos_fit(num_structures, *input_files, eos_index, plot_ev=True, plot_pv=False):
    ...
    if num_structures == 'multiple':
        ...
        sorted_indices = np.argsort(results[:, 2])[::-1] # Sort in terms of minimum energy
        results = results[sorted_indices]
        ...
        if plot_ev and not plot_pv:
            ...
            for key in volume:
                ...
                plt.scatter(results[j][1], results[j][2], facecolors='none', edgecolors='k') # Plot the minimum energy point
                for i in eos_index:
                    if i == 1:
...
[results, energy_eos, pressure_eos] = eos_fit('multiple', *input_files, eos_index=[3], plot_ev=True, plot_pv=False)
